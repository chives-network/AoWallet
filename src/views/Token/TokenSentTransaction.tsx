...
const TokenSentTransaction = (prop: any) => {
  ...
  return (
    <Box
        ...
        <TableContainer>
            ...
            {tokenGetInfor && tokenGetInfor.AoTokenSentTransactionsList && tokenGetInfor.AoTokenSentTransactionsList.map((Item: string[], Index: number)=>{
            ...
            {tokenGetInfor && tokenGetInfor.AoTokenSentTransactionsList == null && (
                <Box sx={{ pl: 5, py: 3 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                    <Grid item key={"Pagination"} xs={12} sm={12} md={12} lg={12} sx={{ padding: '10px 0 10px 0' }}>
                        <CircularProgress />
                        <Typography noWrap variant='body2' sx={{ color: 'primary.main', pr: 3, display: 'inline', ml: 5, pt: 0 }}>{t('Loading token sent txs...')}</Typography>
                    </Grid>
                    </Box>
                </Box>
            )}
...
export default memo(TokenSentTransaction)
