  ...
  useEffect(() => {
    const processWallets = async () => {
      if(isLoadingFinished == false && pageModel == 'Main')  {
        setIsLoading(true)
        try {
          const response = await axios.get(authConfig.backEndApiXwe + '/peersinfo');
          if(response && response.data) {
            const getBlockRewardsData = response.data
            if(getBlockRewardsData && getBlockRewardsData && getBlockRewardsData.length > 0)  {
              setRecords((preV: any)=>(
                [...preV, ...getBlockRewardsData]
              ))
              setTotalRecords(getBlockRewardsData.length)
            }
            if(getBlockRewardsData && getBlockRewardsData && getBlockRewardsData.length == 0) {
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', pl: 2, py: 1}} >
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '60%', ml: 1.5 }}>
                      ...
                      <Box sx={{ display: 'flex' }}>
                        ...
                        >
                          {(Index+1)} {Item.country} {Item.city}
                        </Typography>
                      </Box>
                    </Box>
